...
import {
    ...
    Bus as BusIcon, MapPin, CheckCircle2, ChevronRight,
    Zap, Navigation,
} from 'lucide-react';
...
interface RouteModalProps {
    ...
    onClose: () => void;
    routeId: string | null;
    userStop?: string; // pre-filled from search
}
...
export default function RouteModal({ isOpen, onClose, routeId, userStop }: RouteModalProps) {
    // ── stage: 'select' → user picks a bus | 'track' → confirmed, tracking live
    ...
    // Buses active on this route
    const routeBuses = buses.filter(b => b.routeId === routeId && b.status === 'active');

    // Compute ETA for every bus and sort nearest first
    ...
    // Alerts relevant to this route / the selected bus
    ...
    const trackedBusWithETA = busesWithETA.find(b => b.bus.id === selectedBusId);
    const trackedBusAlerts  = alerts.filter(
        a => a.busId === selectedBusId || a.routeId === routeId
    );

    // Reset on open/close or route change
    useEffect(() => {
        if (!isOpen) {
    ...
    // Auto-select the nearest bus (index 0) when stage opens
    useEffect(() => {
    ...
    // Map markers
    const mapBuses = stage === 'track' && trackedBusWithETA
        ? [{ id: trackedBusWithETA.bus.id, lat: trackedBusWithETA.bus.currentLocation.lat, lng: trackedBusWithETA.bus.currentLocation.lng, label: trackedBusWithETA.bus.busNumber }]
        : busesWithETA.map(b => ({ id: b.bus.id, lat: b.bus.currentLocation.lat, lng: b.bus.currentLocation.lng, label: b.bus.busNumber }));

    const mapCenter = stage === 'track' && trackedBusWithETA
        ? trackedBusWithETA.bus.currentLocation
        : undefined;
            ...
            {isOpen && (
                ...
                >
                    ...
                    >
                        ...
                        {/* ── EMERGENCY BANNER (shown in both stages) ─────────── */}
                        {criticalAlerts.length > 0 && (
                            <div className="flex-none px-5 py-3 bg-red-500/10 border-b border-red-500/20">
                        ...
                        <div className="flex-1 overflow-y-auto">
                                ...
                                <div className="p-4 sm:p-5 space-y-3 sm:space-y-4">
                                    ...
                                    {busesWithETA.length === 0 ? (
                                        ...
                                        <div className="space-y-3">
                                            {busesWithETA.map(({ bus, etaLabel, etaMinutes, nextStop }, idx) => {
                                                ...
                                                return (
                                                    <motion.div
                                                        ...
                                                        className={`relative p-4 rounded-2xl border cursor-pointer transition-all select-none ${
                                                            isSelected
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center gap-3 pr-14">
                                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-lg ${
                                                                idx === 0 ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-700 text-slate-400'
                                                            }`}>
                                                                🚌
                                                            </div>
                                                            <div className="min-w-0">
                                                        ...
                                                        {/* Selection indicator */}
                                                        <div className={`absolute top-1/2 right-4 -translate-y-1/2 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                            ...
                            {stage === 'track' && trackedBusWithETA && (
                                <div className="p-4 sm:p-5 space-y-4">
                                    ...
                                    <div className="bg-slate-800/50 rounded-2xl p-4 border border-slate-800">
                                        <div className="flex items-start justify-between gap-4 flex-wrap">
                                            <div className="flex items-center gap-3">
                                                <div className="w-11 h-11 rounded-full bg-indigo-500/15 flex items-center justify-center text-xl shrink-0">
                                                    🚌
                                                </div>
                                                <div>
                                                    <p className="text-xs text-slate-500 mb-0.5">Tracking bus</p>
                                                    <p className="font-bold text-white">{trackedBusWithETA.bus.busNumber}</p>
                                                </div>
                                            </div>
                                            <button
                                        ...
                                        <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-slate-800">
                                            <div className="text-center">
                                                ...
                                                <p className="font-bold text-white text-sm">{trackedBusWithETA.etaLabel}</p>
                                                <p className="text-slate-500 text-[10px] truncate">{trackedBusWithETA.nextStop}</p>
                                            </div>
                                            <div className="text-center">
                                                <p className="text-slate-500 text-xs mb-1 flex items-center justify-center gap-1"><Users size={10} /> Seats</p>
                                                <p className={`font-bold text-sm ${(trackedBusWithETA.bus.capacity - trackedBusWithETA.bus.passengerCount) < 5 ? 'text-red-400' : 'text-emerald-400'}`}>
                                                    {trackedBusWithETA.bus.capacity - trackedBusWithETA.bus.passengerCount}
                                                </p>
                                                <p className="text-slate-500 text-[10px]">of {trackedBusWithETA.bus.capacity}</p>
                                            </div>
                                            <div className="text-center">
                                                <p className="text-slate-500 text-xs mb-1 flex items-center justify-center gap-1"><Navigation size={10} /> Speed</p>
                                                <p className="font-bold text-white text-sm">{trackedBusWithETA.bus.speed} km/h</p>
                                            </div>
                                        </div>
                                    </div>

                                    {/* Alerts for this bus/route */}
                                    <div>
                                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Live Notifications</p>
                                        {trackedBusAlerts.length === 0 ? (
                                    ...
                                    {/* Stops on this route */}
                                    <div>
                                        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Route Stops</p>
                                        <div className="space-y-2">
                                            {route.stops.map((stop, i) => (
                                                <div key={stop.name} className="flex items-center gap-3">
                                                    <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                                                        i === 0 ? 'bg-emerald-400' :
                                                        i === route.stops.length - 1 ? 'bg-red-400' :
                                                        'bg-slate-600'
                                                    }`} />
                                                    <span className={`text-sm ${
                                                        stop.name === userStop ? 'text-indigo-300 font-bold' : 'text-slate-400'
                                                    }`}>
                                                        {stop.name}
                                                        {stop.name === userStop && <span className="ml-2 text-[10px] bg-indigo-500/20 text-indigo-400 px-1.5 py-0.5 rounded">Your Stop</span>}
                                                    </span>
                                                </div>
                                            ))}
                                        </div>
